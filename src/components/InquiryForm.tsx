...
export default function InquiryForm({ tripId, tripTitle }: InquiryFormProps) {
  ...
  if (submitted) {
    return (
      <div className="rounded-xl border border-green-200 bg-green-50 p-6 text-center">
        ...
        <h3 className="text-lg font-bold text-green-600">已收到您的諮詢！</h3>
        <p className="mt-2 text-sm text-gray-600">蓋瑞會盡快與您聯繫，謝謝！</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-xl border border-gray-200 bg-white p-6">
      <h3 className="mb-4 text-lg font-bold text-gray-900">線上諮詢表單</h3>
      <div className="space-y-4">
        <div>
          ...
          <input
            ...
            required
            placeholder="您的姓名"
            className="w-full rounded-lg border border-gray-200 bg-gray-50 px-4 py-2.5 text-gray-900 placeholder-gray-400 transition focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500"
          />
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-600">電話</label>
            <input
              type="tel"
              value={phone}
        ...
        <div>
          <label className="mb-1 block text-sm font-medium text-gray-600">留言</label>
          <textarea
            value={message}
