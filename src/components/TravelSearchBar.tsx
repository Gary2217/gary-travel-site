...
export default function TravelSearchBar({ regions, onSearch }: TravelSearchBarProps) {
  ...
  function formatEzDate(d: string) {
    if (!d) return "";
    const m = d.slice(5, 7);
  ...
  function handleFlightSearch() {
    const to = toCity?.code ?? "";
    const type = tripType === "roundtrip" ? "roundtrip" : "oneway";
    const base = `https://flight.eztravel.com.tw/tickets-${type}-${fromCode}-${to}/`;
    const params = new URLSearchParams({
      outbounddate: formatEzDate(outboundDate),
      adults: "1",
      children: "0",
      infants: "0",
      direct: "false",
    ...
    if (tripType === "roundtrip") params.set("inbounddate", formatEzDate(inboundDate));
