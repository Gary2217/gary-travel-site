...
import { getBackHint } from "@/lib/trip-format";

interface DownloadGateModalProps {
  open: boolean;
  /** 是否已追蹤過社群（顯示「感謝追蹤」+ 下載按鈕） */
...
export default function DownloadGateModal({
  ...
  const followAndOpen = (platform: "LINE" | "FB" | "IG", url: string) => {
    track({ event_type: "trip_download", platform, trip_id: tripId, trip_title: tripTitle });
    onReady();
        ...
        {!downloadReady ? (
          <>
            ...
            <div className="space-y-2">
              <a
                href={lineHref}
                onClick={(e) => { e.preventDefault(); followAndOpen("LINE", lineHref); }}
              ...
              <a
                href={fbHref}
                onClick={(e) => { e.preventDefault(); followAndOpen("FB", fbHref); }}
              ...
              <a
                href={igHref}
                onClick={(e) => { e.preventDefault(); followAndOpen("IG", igHref); }}
