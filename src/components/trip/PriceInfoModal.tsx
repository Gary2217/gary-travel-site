...
interface PriceInfoModalProps {
  open: boolean;
  /**
   * 已存檔的售價明細（行程頁的 priceDetailPreview，來自 selectedDepartureInfo.price_detail）。
   * 注意：不可傳開發者編輯中的 detail* 草稿，否則客人會看到還沒存檔的內容。
   */
...
/** 使用者端「售價說明」彈窗 */
export default function PriceInfoModal({ open, detail, onClose }: PriceInfoModalProps) {
  if (!open) return null;
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-x-4 gap-y-2 rounded-xl border border-gray-100 bg-gray-50 px-4 py-3 text-sm">
          <div>
            ...
            <div className="font-bold text-sky-600">{displayAdultUnit(detail.adultPrice)}</div>
          </div>
          <div>
            <div className="text-[11px] text-gray-500">小孩佔床</div>
            <div className="font-bold text-gray-900">{displayChildPrice(detail.childWithBedPrice)}</div>
          </div>
          <div>
            <div className="text-[11px] text-gray-500">小孩不佔床</div>
            <div className="font-bold text-sky-600">{displayChildPrice(detail.childNoBedPrice)}</div>
          </div>
          <div>
            <div className="text-[11px] text-gray-500">加床</div>
            <div className="font-bold text-gray-900">{displayChildPrice(detail.childExtraBedPrice)}</div>
          </div>
          <div>
            <div className="text-[11px] text-gray-500">嬰兒</div>
            <div className="font-bold text-sky-600">{displayInfantUnit(detail.infantPrice)}</div>
          </div>
          <div>
            <div className="text-[11px] text-gray-500">單人房差</div>
            <div className="font-bold text-sky-600">{formatSingleRoomText(detail.singleRoom)}</div>
          </div>
        </div>
        ...
        <div className="mt-3 space-y-2 text-sm">
          {(detail.surcharge || detail.visaFee) && (
            <div className="flex gap-2">
              ...
              <span className="text-gray-600">{[detail.surcharge, detail.visaFee].filter(v => v && v !== '售價已內含' && v !== '免簽證').join('，') || '含機場稅燃油附加費'}</span>
            </div>
          )}
