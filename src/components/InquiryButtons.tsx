...
export default function InquiryButtons({ tripTitle, variant }: InquiryButtonsProps) {
  const [toast, setToast] = useState<string | null>(null);

  const copyAndOpen = useCallback(async (url: string, platform: string) => {
    ...
    return (
      <>
        ...
        <div className="fixed bottom-4 right-3 z-[9998] flex flex-col items-end gap-1.5 sm:bottom-20 sm:right-4 sm:gap-2">
          ...
          <button
            onClick={() => copyAndOpen(lineDmHref, "LINE")}
          ...
          <button
            onClick={() => copyAndOpen(fbDmHref, "Facebook")}
          ...
          <button
            onClick={() => copyAndOpen(igDmHref, "Instagram")}
    ...
    <>
      ...
      <div className="rounded-xl border border-white/10 bg-[rgba(20,20,30,0.38)] px-2 py-0.25 backdrop-blur-[12px] md:px-3 md:py-0.5">
        ...
        <div className="flex flex-col items-center gap-0.5 sm:flex-row sm:justify-center md:gap-1">
          <button
            onClick={() => copyAndOpen(lineDmHref, "LINE")}
            className="inline-flex h-8 w-full items-center justify-center gap-2 rounded-full bg-[#06C755] px-4 text-[11px] font-semibold text-white shadow-sm transition hover:bg-[#05b64d] sm:w-auto md:h-8.5 md:min-w-[128px] md:px-4"
          >
          ...
          <button
            onClick={() => copyAndOpen(fbDmHref, "Facebook")}
            className="inline-flex h-8 w-full items-center justify-center gap-2 rounded-full bg-[#1877F2] px-4 text-[11px] font-semibold text-white shadow-sm transition hover:bg-[#1565d8] sm:w-auto md:h-8.5 md:min-w-[128px] md:px-4"
          >
          ...
          <button
            onClick={() => copyAndOpen(igDmHref, "Instagram")}
            className="inline-flex h-8 w-full items-center justify-center gap-2 rounded-full bg-[#E4405F] px-4 text-[11px] font-semibold text-white shadow-sm transition hover:bg-[#d62d4a] sm:w-auto md:h-8.5 md:min-w-[128px] md:px-4"
          >
