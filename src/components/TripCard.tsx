...
export default function TripCard({
  ...
  const shareToLine = () => {
    window.open(`https://social-plugins.line.me/lineit/share?url=${encodeURIComponent(tripPageUrl)}`, '_blank');
  };

  const shareToFb = () => {
    window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(tripPageUrl)}`, '_blank');
  };

  const shareToIg = () => {
    navigator.clipboard.writeText(tripPageUrl).then(() => {
      alert("已複製行程連結！請打開 Instagram 貼上分享給好友");
    }).catch(() => {
      alert(`請複製此連結分享：${tripPageUrl}`);
    });
  };
        ...
        <div className="p-2 sm:p-3 md:p-4">
          ...
          {!isDevMode && (
            <div className="mt-1.5 flex items-center justify-center gap-2 sm:mt-2">
              <span className="text-[10px] text-white/40 sm:text-[11px]">分享</span>
              <button
                onClick={shareToLine}
                className="flex h-6 w-6 items-center justify-center rounded-full bg-[#06C755]/20 text-[#06C755] transition hover:bg-[#06C755]/40 active:scale-90 sm:h-7 sm:w-7"
                title="LINE 分享給好友"
              >
                <svg className="h-3 w-3 sm:h-3.5 sm:w-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 10.314C24 4.943 18.615.572 12 .572S0 4.943 0 10.314c0 4.811 4.27 8.842 10.035 9.608.391.082.923.258 1.058.59.12.301.079.766.038 1.08l-.164 1.02c-.045.301-.24 1.186 1.049.645 1.291-.539 6.916-4.078 9.436-6.975C23.176 14.393 24 12.458 24 10.314" /></svg>
              </button>
              <button
                onClick={shareToFb}
                className="flex h-6 w-6 items-center justify-center rounded-full bg-[#1877F2]/20 text-[#1877F2] transition hover:bg-[#1877F2]/40 active:scale-90 sm:h-7 sm:w-7"
                title="Facebook 分享給好友"
              >
                <svg className="h-3 w-3 sm:h-3.5 sm:w-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" /></svg>
              </button>
              <button
                onClick={shareToIg}
                className="flex h-6 w-6 items-center justify-center rounded-full bg-[#E4405F]/20 text-[#E4405F] transition hover:bg-[#E4405F]/40 active:scale-90 sm:h-7 sm:w-7"
                title="Instagram 分享給好友"
              >
                <svg className="h-3 w-3 sm:h-3.5 sm:w-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" /></svg>
              </button>
            </div>
          )}
        </div>
