...
export default function LegalNotice({ className = "", variant = "light" }: LegalNoticeProps) {
  ...
  return (
    <section className={className}>
      ...
      <p className={`mt-2 text-[10px] leading-relaxed ${isDark ? "text-white/70" : "text-gray-400"}`}>
        本網站透過「聯絡我們」表單蒐集之姓名、電話、LINE ID、信箱等個人資料，僅用於回覆您的旅遊諮詢，不會提供給任何第三方。您可隨時要求查閱、更正或刪除個人資料。詳見<Link href="/privacy" className={isDark ? "text-white underline hover:text-white/80" : "text-sky-500 underline hover:text-sky-400"}>隱私權政策</Link>。
      </p>
    </section>
  );
}
